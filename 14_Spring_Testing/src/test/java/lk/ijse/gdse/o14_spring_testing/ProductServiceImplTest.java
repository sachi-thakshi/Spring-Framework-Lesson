...
public class ProductServiceImplTest {
    @Mock // create a fake repository class
    private ProductRepository productRepository;

    @InjectMocks // inject a real one
    private ProductServiceImpl productService;

    private Product product;

    @BeforeEach // hamaekakatama kalin run wenna
    public void setUp(){
    ...
    void shouldSaveProduct(){
        ...
        // asserts
        Assertions.assertNotNull(savedProduct); // assertNotNull -> null neweida balanwa product eka
        Assertions.assertEquals(product, savedProduct); // assertEquals -> dunnu ekamada save wune balanwa
//        Assertions.assertEquals(1L, product.getId());
        verify(productRepository, times(1)).save(product); // kochchara paarak invocation wenna onida kiyala
    }
